  ...
  return (
    <div style={{ display: 'flex', gap: 4, width: '100%' }}>
      {[1, 2, 3, 4].map((i) => (
        <span
          key={i}
          style={{ flex: 1, height: 3, borderRadius: 2, background: i <= strength ? colors[strength] : T.line, display: 'block' }}
        />
      ))}
    </div>
  );
...
function strengthColor(len: number) {
  if (len === 0) return T.mute;
  if (len < 6) return T.err;
  if (len < 10) return T.amber;
  if (len < 14) return T.lime;
  return T.ok;
}
...
export function Screen2({ onNext }: Screen2Props) {
  ...
  const domainVerified = email.includes('@') && (email.split('@')[1]?.includes('.') ?? false);
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: T.bg }}>
      ...
      {/* Two-div scroll pattern: outer clips, inner centers with min-height fallback */}
      <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
        <div style={{
          ...
          <div style={{
            ...
            <h2 style={{ fontSize: 24, fontWeight: 700, color: T.ink, margin: '0 0 6px', letterSpacing: -0.5, fontFamily: T.sans }}>
              Create your account
            </h2>
            <p style={{ fontSize: 13, color: T.mute, margin: '0 0 24px', lineHeight: 1.5, fontFamily: T.sans }}>
            ...
            <div style={{ display: 'grid', gap: 14 }}>
              ...
              <Field
                ...
                right={
                  password
                    ? <span style={{ textTransform: 'none', color: strengthColor(password.length), fontWeight: 500, fontFamily: T.sans, fontSize: 11 }}>
            ...
            {/* Checkbox comes before its own error message */}
            <div style={{ marginTop: 20 }}>
              ...
              />
              {errors.agreed && (
                <p style={{ fontSize: 11, color: T.err, margin: '6px 0 0 26px', fontFamily: T.sans }}>{errors.agreed}</p>
              )}
            </div>

            <div style={{ marginTop: 20 }}>
              <PrimaryBtn full icon="→" onClick={() => { if (validate()) onNext(); }}>Create Account</PrimaryBtn>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <GhostBtn full>
                <svg width="14" height="14" viewBox="0 0 16 16" fill={T.ink} aria-label="GitHub">
            ...
            <p style={{ fontSize: 11.5, color: T.mute, textAlign: 'center', margin: '24px 0 0', fontFamily: T.sans }}>
